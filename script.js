...
// 🎯 重新校正分流標籤：center (運動中心)、school (國中小校園籃球)、court (獨立羽網球場)
const sportsDatabase = {
  ...
  "宜蘭縣": [
    ...
    { name: "宜蘭市立運動公園籃球場", address: "宜蘭縣宜蘭市中山路五段50號", hours: "24小時營業", type: "school", typeText: "籃球/體育館/校園" },
...
function searchSports() {
  ...
  setTimeout(() => {
    ...
    // 🎯 核心修正點：比對正確的簡寫代碼（all / center / school / court）
    ...
    if (filteredVenues.length === 0) {
      resultsDiv.innerHTML = `<p style="color:#e74c3c; text-align:center; font-weight:bold; padding: 20px;">❌ 該縣市在此分類下暫無登記資料，請換一個運動類型試試看！</p>`;
      return;
    }
    ...
    filteredVenues.forEach((venue) => {
      ...
      // 🛠️ 內嵌地圖專用網址（帶有正確的錢字號變數注入）
      const embedMapUrl = `https://maps.google.com/maps?q=${encodeURIComponent(searchQuery)}&t=&z=15&ie=UTF8&iwloc=&output=embed`;
      
      // 🎯 【新增功能】直接導向該地址的 Google Map 實體連結網址
      const addressGoogleMapUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedCity + venue.address)}`;
      
      const card = document.createElement('div');
      card.className = 'card';
      card.innerHTML = `
        <h3>📍 ${venue.name} <span class="tag tag-${venue.type}">${venue.typeText}</span></h3>
        
        <p><strong>🏠 詳細地址：</strong> 
          <a href="${addressGoogleMapUrl}" target="_blank" class="review-link" title="點擊在 Google Map 打開地址">
            ${venue.address} 🔗 (導航路徑)
          </a>
        </p>
